...
async function getWord() {
	...
	url.searchParams.set("w", currentWord);
	window.history.pushState({}, (currentWord + " Definition — Stationæry"), url);
	...
	if (hist === null) {
		localStorage.setItem("hist", currentWord);
	} else {
		hist = hist.split(",");
		if (hist.includes(currentWord)) {
			hist.splice(hist.indexOf(currentWord), 1);
		}
		hist.unshift(currentWord);
